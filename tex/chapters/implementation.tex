...
\begin{algorithmic}[1]
\Require Seznam $P$ dvojic $(c, x)$, kde $c$ je omezující podmínka definovaná nad proměnnou $x$.
...
\State $D_x' \gets$ Zredukuj doménu $D_x$ proměnné $x \in p$ podle podmínky $c$.
\If{$D_x' = \emptyset $}
...
\State $P' \gets HC3(P)$
\State $(P_1, P_2) \gets $ rozděl box tvořený doménami proměnných z $P'$ na poloviny.
\State Solve($P_1$)
\State Solve($P_2$)
\Else
