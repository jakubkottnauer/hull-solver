...
\abstractEN{A constraint safisfaction problem (CSP) is a~problem of finding the~values of variables while satisfying all the given constraints. A constraint is a relation between variables which reduces the set of values that can be assigned to a variable. The~main goal of this thesis is to test the influence of various heuristics on the efficiency of solving numerical CSP problems by interval propagation. A~problem solver written in the F\# language utilizing the HC3 algorithm and a branch-and-prune algorithm has been created, important aspects of the HC3 algorithm influencing its efficiency have been found and then experiments with \textbf{NUMBER} different heuristics were performed. The results found in the thesis can be used when deciding which heuristic to use for solving constraint systems.}
